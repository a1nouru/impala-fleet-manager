...
export default function LoansPage() {
  ...
  const saveLoan = async () => {
    ...
    if (!form.borrower_name.trim()) {
      toast({
        ...
        description: t(
          "loans.messages.borrowerRequiredDesc",
          "Enter the name of the person or company receiving the loan."
        ),
        variant: "destructive",
      });
      return;
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      toast({
        ...
        description: t(
          "loans.messages.amountRequiredDesc",
    ...
    try {
      ...
      if (editingLoan) {
        ...
        toast({
          title: t("loans.messages.updated", "Loan updated"),
          description: t("loans.messages.updatedDesc", "The loan details were saved."),
        });
      } else {
        await loanService.createLoan(payload, user?.email || "");
        toast({
          title: t("loans.messages.created", "Loan recorded"),
          description: t("loans.messages.createdDesc", "The loan was added to the register."),
        });
      }
      setFormOpen(false);
      setEditingLoan(null);
  ...
  const confirmClose = async () => {
    ...
    try {
      ...
      toast({
        title: t("loans.messages.closed", "Loan closed"),
        description: t("loans.messages.closedDesc", "The loan is now marked as paid."),
      });
      setCloseTarget(null);
      await loadLoans();
    } catch (error) {
  ...
  const reopen = async (loan: Loan) => {
    try {
      ...
      toast({
        title: t("loans.messages.reopened", "Loan reopened"),
        description: t("loans.messages.reopenedDesc", "The loan is open again."),
      });
      await loadLoans();
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast({
        title: t("loans.messages.deleted", "Loan deleted"),
        description: t("loans.messages.deletedDesc", "The loan was removed from the register."),
      });
